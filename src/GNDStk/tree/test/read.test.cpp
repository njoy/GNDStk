...
SCENARIO("Testing GNDStk tree read() and operator>>") {
   ...
   GIVEN("Some trees") {
      ...
      WHEN("We read one from an istream, specifically a stringstream") {
         ...
         sstr << ifs.rdbuf();
         t4.read(sstr);
         REQUIRE(t4.root != nullptr); // we get something
      }

      WHEN("We stream-input one, specifically from an ifstream") {
         std::ifstream ifs("n-008_O_016.xml");
         ifs >> t5;
         REQUIRE(t5.root != nullptr); // we get something
      }
   }

   // Why not...
   GIVEN("Several trees, read differently, but ultimately from the same file") {
      // While this test isn't intended to test tree's write(), let's
      // nevertheless do a simple write() to tree's basic text-output
      // format, from each of the trees read in different ways above,
      // and be sure there are absolutely no differences between them.
   ...
   // fixme There's actually a great deal more we can test, with regards
   // to tree's read(). Above, we're just dealing with an XML file, not
   // a Json (for which we don't yet have an official GNDS file) or
   // an HDF (ditto, and don't have read/write capabilities yet either).
   // Also, we allow for a direct user-stipulated format (xml, json, hdf5)
   // in our read()s, for which warnings might be issued if the file
   // extension and/or the file's first byte appear to be inconsistent
   // with the user-stipulated format.
}
