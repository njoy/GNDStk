
// THIS FILE WAS AUTOGENERATED!
// DO NOT MODIFY!

#ifndef NJOY_GNDSTK_V2_0_TSL_E_CRITICAL
#define NJOY_GNDSTK_V2_0_TSL_E_CRITICAL

// core interface
#include "GNDStk.hpp"

namespace njoy {
namespace GNDStk {
namespace v2_0 {

using namespace njoy::GNDStk::core;



// -----------------------------------------------------------------------------
// tsl::
// class E_critical
// -----------------------------------------------------------------------------

namespace tsl {

class E_critical : public Component<E_critical> {


   // ------------------------
   // For Component
   // ------------------------

   friend class Component;

   // Current namespace, current class, and GNDS node name
   static auto namespaceName() { return "tsl"; }
   static auto className() { return "E_critical"; }
   static auto GNDSName() { return "e_critical"; }

   // Core Interface multi-query to extract metadata and child nodes
   static auto keys()
   {
      return
         // metadata
         std::optional<XMLName>{}
            / Meta<>("unit") |
         std::optional<Float64>{}
            / Meta<>("value")
      ;
   }

public:

   using Component::construct;

   // ------------------------
   // Relevant defaults
   // FYI for users
   // ------------------------

   static inline const struct Defaults {
   } defaults;

   // ------------------------
   // Raw GNDS content
   // ------------------------

   struct {
      // metadata
      std::optional<XMLName> unit;
      std::optional<Float64> value;
   } content;

   // ------------------------
   // Getters
   // const and non-const
   // ------------------------

   // unit
   const std::optional<XMLName> &unit() const
      { return content.unit; }
   std::optional<XMLName> &unit()
      { return content.unit; }

   // value
   const std::optional<Float64> &value() const
      { return content.value; }
   std::optional<Float64> &value()
      { return content.value; }

   // ------------------------
   // Setters
   // non-const
   // All return *this
   // ------------------------

   // unit(value)
   E_critical &unit(const std::optional<XMLName> &obj)
      { unit() = obj; return *this; }

   // value(value)
   E_critical &value(const std::optional<Float64> &obj)
      { value() = obj; return *this; }

   // ------------------------
   // Construction
   // ------------------------

   // default
   E_critical() :
      Component{
         BodyText{},
         content.unit,
         content.value
      }
   {
      Component::finish();
   }

   // copy
   E_critical(const E_critical &other) :
      Component{
         other,
         content.unit,
         content.value
      },
      content{other.content}
   {
      Component::finish(other);
   }

   // move
   E_critical(E_critical &&other) :
      Component{
         other,
         content.unit,
         content.value
      },
      content{std::move(other.content)}
   {
      Component::finish(other);
   }

   // from node
   E_critical(const Node &node) :
      Component{
         BodyText{},
         content.unit,
         content.value
      }
   {
      Component::finish(node);
   }

   // from fields
   explicit E_critical(
      const std::optional<XMLName> &unit,
      const std::optional<Float64> &value
   ) :
      Component{
         BodyText{},
         content.unit,
         content.value
      },
      content{
         unit,
         value
      }
   {
      Component::finish();
   }

   // ------------------------
   // Assignment
   // ------------------------

   // copy
   E_critical &operator=(const E_critical &) = default;

   // move
   E_critical &operator=(E_critical &&) = default;

   // ------------------------
   // Custom functionality
   // ------------------------

   #include "GNDStk/v2.0/tsl/E_critical/src/custom.hpp"

}; // class E_critical

} // namespace tsl
} // namespace v2_0
} // namespace GNDStk
} // namespace njoy

#endif
