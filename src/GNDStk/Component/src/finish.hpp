...
by our autogeneration tool can run additional code that someone's customizations
of their autogenerated classes might need.
...
but it doesn't go into a generated field in the derived class. (If it did go
into a generated field, then we'd simply extract it from there, instead of
having finish() receive it as a parameter.)
...
void construct() { }
void construct(const DERIVED &) { derived().construct(); }
void construct(const Node &) { derived().construct(); }
template<class T>
void construct(const std::vector<T> &) { derived().construct(); }
