...
by casting first: static_cast<DERIVED &>(*this).
...
Specifically, here's what the constructors in the auto-generated classes call:
...
void finish()
{
   ...
   BodyText<hasBodyText>::pullFromDerived(derived.content);
...
{
   // First, read data fields from the Node into the derived object's fields.
   // Basically, this applies the keys() multi-query in the derived class.
   // The following call also runs BodyText::fromNode(), which gets a raw
   // string of "body text" if the Node has body text.
   fromNode(node);
