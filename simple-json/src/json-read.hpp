...
// we provide into a "longer" type. Also, read() lets specific integral and/or
   ...
   std::apply(
      [this,&tok,&found](const auto &... alt)
      {
         ...
         (
            (
               found || (found =
             !(gaveIntegral && json::integral<decltype(alt)>) &&
             !(gaveFloating && json::floating<decltype(alt)>) &&
